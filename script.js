...
const renderInitialCustomerSelectionModal = () => {
    const modal = document.getElementById('initialCustomerModal');
    const container = document.getElementById('initialCustomerListUI');
    const globalBtn = document.getElementById('initialSelectGlobalBtn');
    const addBtn = document.getElementById('initialAddNewCustBtn');
    ...
    customers.forEach(cust => {
        ...
        card.className = 'initial-cust-card';
        card.innerHTML = `
            <div style="font-weight: 700; font-size: 1.05rem; color: var(--text-color);">👤 ${cust.name}</div>
            ${cust.phone ? `<div style="font-size: 0.85rem; color: var(--subtle-text-color); margin-top: 2px;">📞 ${cust.phone}</div>` : ''}
        `;
        card.onclick = () => {
            selectActiveCustomer(cust.id);
            modal.style.display = 'none';
        };
...
const initDevModeModule = () => {
    ...
    // Dev Migration Button: Tag All Legacy Data to Rajesh Ji
    const migrateRajeshBtn = document.getElementById('devMigrateRajeshBtn');
    if (migrateRajeshBtn) {
        migrateRajeshBtn.addEventListener('click', async () => {
            if (!db || !user) return alert("Please make sure you are logged in and online.");
            const rajeshId = getRajeshCustomerId();
            const rajeshName = 'Rajesh Ji Powakhali';

            const confirmed = await showConfirm(
                "Tag All Legacy Data", 
                "This will scan Cloud Firestore and tag all existing untagged inventory loans, reports, and dues as belonging to 'Rajesh Ji Powakhali'. Continue?"
            );

            if (!confirmed) return;

            showConfirm("Tagging Legacy Data...", "Updating untagged loans and reports in Cloud Firestore...", false);
            ...
            try {
                ...
                await loadFinalisedTransactions();
                refreshActiveViewForCustomer();
                alert(`Successfully tagged ${updatedCount} legacy documents to Rajesh Ji Powakhali!`);

            } catch (err) {
                closeConfirm();
                console.error("Migration error:", err);
                alert("Error tagging legacy data: " + err.message);
            }
        });
    }

    // Dev Backup & Restore Button Handlers
    const backupBtn = document.getElementById('devBackupBtn');
    const restoreBtn = document.getElementById('devRestoreBtn');
    const restoreFileInput = document.getElementById('devRestoreFileInput');
    ...
    if (backupBtn) {
        backupBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                a.href = url;
                a.download = `firebase_interest_calc_backup_${formatDateToDDMMYYYY(new Date()).replace(/\//g, '-')}.json`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                URL.revokeObjectURL(url);

                alert(`Backup downloaded successfully! Included ${backup.activeInventory.length} inventory loans and ${backup.sharedReports.length} reports.`);
            } catch (err) {
                closeConfirm();
    ...
    if (restoreBtn && restoreFileInput) {
        ...
        restoreFileInput.addEventListener('change', async (e) => {
            ...
            const confirmed = await showConfirm(
                ...
                "This will restore inventory loans, reports, and dues from the selected JSON backup file into Cloud Firestore. Continue?"
            );

            if (!confirmed) return;

            showConfirm("Restoring Database...", "Uploading records into Cloud Firestore...", false);
            ...
            try {
                ...
                reader.onload = async (evt) => {
                    try {
                        ...
                        // 1. Restore Inventory
                        if (backup.activeInventory && Array.isArray(backup.activeInventory)) {
                            const batch = db.batch();
                            let count = 0;
                            backup.activeInventory.forEach(item => {
                                const { id, ...data } = item;
                                if (id) {
                                    batch.set(db.collection('activeInventory').doc(id), data, { merge: true });
                                    count++;
                                    restoredCount++;
                                }
                            });
                        ...
                        // 2. Restore Shared Reports
                        if (backup.sharedReports && Array.isArray(backup.sharedReports)) {
                            const batch = db.batch();
                            let count = 0;
                            backup.sharedReports.forEach(report => {
                                const { id, ...data } = report;
                                if (id) {
                                    batch.set(db.collection('sharedReports').doc(id), data, { merge: true });
                                    count++;
                                    restoredCount++;
                                }
                            });
                        ...
                        // 3. Restore Dues
                        if (backup.customerDues && Array.isArray(backup.customerDues)) {
                            const batch = db.batch();
                            let count = 0;
                            backup.customerDues.forEach(due => {
                                const { id, ...data } = due;
                                if (id) {
                                    batch.set(db.collection('customerDues').doc(id), data, { merge: true });
                                    count++;
                                    restoredCount++;
                                }
                            });
                        ...
                        await loadFinalisedTransactions();
                        refreshActiveViewForCustomer();
                        alert(`Successfully restored ${restoredCount} database records!`);

                    } catch (jsonErr) {
                        closeConfirm();
                        alert("Invalid backup file: " + jsonErr.message);
                    }
                };
