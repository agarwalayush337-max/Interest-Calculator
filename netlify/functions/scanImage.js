...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (scanType === 'loan_numbers') {
        ...
        promptText = `From the provided image, identify all loan entries. For each entry, extract the 'LoanNo', 'Principal', and 'Date'. Return the result as a clean JSON array of objects where each object has the keys "no", "principal", and "date".Format the 'Date' field into a 'DD/MM/YYYY' string. Format the 'no' field by replacing any '.',' ','-' with a '/',And if there is nothing between Alphabet And number in 'no' field then add '/'.if a loan number starts with a letter followed immediately by the digit '1' and TWO other digits (e.g., A1531), you must **replace** the '1' with a '/' to get a result like 'A/531'. Do not just add a slash.Do not include any text, explanations, or markdown formatting in your response, only the raw JSON array.`;
    }
